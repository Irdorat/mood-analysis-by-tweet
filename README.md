...
4) Запускать файлы из папки notebooks и обучать модели
...
|Размер модели| 903 Кб|120 Мб|119 Мб|
...
## Вывод
### CatBoost — лучшая по результатам
Даёт +3.5% к accuracy относительно логрегрессии → это очень хороший прирост

F1 тоже вырос → модель стала лучше сбалансирована

Precision ↑ → меньше ложных срабатываний

### Logistic Regression

Всего -3% от CatBoost, но: модель в 130 раз меньше -> быстрее и проще
